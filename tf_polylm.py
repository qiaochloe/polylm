...
class PolyLMModel:
    ...
    def __init__(self, vocab, n_senses, options, training=False):
        ...
        self._unmasked_seqs = tf.compat.v1.placeholder(tf.int32, [None, None])
        self._masked_seqs = tf.compat.v1.placeholder(tf.int32, [None, None])
        self._padding = tf.compat.v1.placeholder(tf.int32, [None, None])
        self._targets = tf.compat.v1.placeholder(tf.int32, [None])
        self._target_positions = tf.compat.v1.placeholder(tf.int32, [None])
        self._dl_r = tf.compat.v1.placeholder(tf.float32, [])
        self._ml_coeff = tf.compat.v1.placeholder(tf.float32, [])

        self._total_senses = np.sum(n_senses) + 1
        sense_indices = np.zeros([self._vocab.size, self._max_senses], dtype=np.int32)
        sense_to_token = np.zeros([self._total_senses], dtype=np.int32)
        sense_to_sense_num = np.zeros([self._total_senses], dtype=np.int32)
        sense_mask = np.zeros([self._vocab.size, self._max_senses], dtype=np.float32)
        ...
        embedding_initializer = tf.compat.v1.random_normal_initializer(
            0.0, 1.0 / np.sqrt(self._embedding_size)
        )
        self._embeddings = tf.compat.v1.get_variable(
            "embeddings",
            [self._total_senses, self._embedding_size],
            dtype=tf.float32,
            initializer=embedding_initializer,
        )
        self._biases = tf.compat.v1.get_variable(
            "biases",
        ...
        self._sense_weight_logits = tf.compat.v1.get_variable(
            "sense_weights",
        ...
        no_predict_tokens = [
            ...
            self._vocab.pad_vocab_id,
            self._vocab.mask_vocab_id,
        ]
        unpredictable_tokens = np.zeros([self._total_senses], dtype=np.float32)
        for t in no_predict_tokens:
            unpredictable_tokens[sense_indices[t, 0]] = 1.0
        self._unpredictable_tokens = tf.constant(unpredictable_tokens)

        mean_prob = sense_mask / np.sum(sense_mask, axis=1, keepdims=True)
        self._mean_qp = tf.compat.v1.get_variable(
            "mean_qp",
            [self._vocab.size, self._max_senses],
            initializer=tf.compat.v1.constant_initializer(mean_prob),
            trainable=False,
        )
        self._mean_qd = tf.compat.v1.get_variable(
            "mean_qd",
            [self._vocab.size, self._max_senses],
            initializer=tf.compat.v1.constant_initializer(mean_prob),
            trainable=False,
        ...
        if options.use_disambiguation_layer:
            self._disambiguated_reps, _ = self._disambiguation_layer(self._masked_seqs)
            _, self._qd = self._disambiguation_layer(self._unmasked_seqs)
            self._qd = tf.reshape(self._qd, [-1, self._max_senses])
            self._qd = tf.nn.embedding_lookup(self._qd, self._target_positions)
        else:
            pass
        ...
        flattened_reps = tf.reshape(self._output_reps, [-1, self._embedding_size])
        ...
        target_position_scores = (
            tf.matmul(self._target_reps, self._embeddings, transpose_b=True)
        ...
        self.target_sense_probs = tf.gather(
            target_position_probs, target_sense_indices, batch_dims=1
        )
        target_sense_masks = tf.nn.embedding_lookup(self._sense_mask, self._targets)
        ...
        log_target_probs = tf.math.log(self.target_token_probs)
        ...
        log_sharpened_q = tf.math.log(tf.reduce_sum(sharpened_q, axis=1))
        log_sharpened_q = log_sharpened_q * targets_are_multisense
        self.d_loss = -tf.reduce_sum(log_sharpened_q) / (self._dl_r * n_multisense)
        ...
        if self._has_disambiguation_layer:
            ...
            self.m_loss = -self._ml_coeff * tf.reduce_sum(cosine_sim) / n_multisense

            self._update_mean_qd = self._update_smoothed_mean(
                self._mean_qd, qd, indices=tf.expand_dims(self._targets, axis=1)
            )
        else:
            self.m_loss = 0.0
        ...
        self._update_mean_qp = self._update_smoothed_mean(
            self._mean_qp, qp, indices=tf.expand_dims(self._targets, axis=1)
        )
    ...
    def _get_sense_embeddings(self, tokens):
        sense_indices = tf.nn.embedding_lookup(self._sense_indices, tokens)
        return tf.nn.embedding_lookup(self._embeddings, sense_indices)

    def _get_sense_embeddings_and_biases(self, tokens):
        sense_indices = tf.nn.embedding_lookup(self._sense_indices, tokens)
        sense_embeddings = tf.nn.embedding_lookup(self._embeddings, sense_indices)
        sense_biases = tf.nn.embedding_lookup(self._biases_with_dummy, sense_indices)
    ...
    def _make_word_embeddings(self, seqs, sense_weights=None):
        ...
        if sense_weights is None:
            ...
            sense_mask = tf.nn.embedding_lookup(self._sense_mask, seqs)
    ...
    def _calculate_sense_probs(self, seqs, reps):
        # ids.shape + (n_senses, embedding_size)
        sense_embeddings, sense_biases = self._get_sense_embeddings_and_biases(seqs)
        # ids.shape + (n_senses, 1)
        sense_scores = tf.matmul(sense_embeddings, tf.expand_dims(reps, axis=-1))
        # ids.shape + (n_senses)
    ...
    def _disambiguation_layer(self, seqs):
        with tf.compat.v1.variable_scope("disambiguation"):
            word_embeddings = self._make_word_embeddings(seqs)

            model = bert.BertModel(
                self._disambiguation_bert_config,
                self._training,
                word_embeddings,
                self._padding,
            )
    ...
    def _prediction_layer(self, reps):
        with tf.compat.v1.variable_scope("prediction"):
            model = bert.BertModel(
                self._prediction_bert_config, self._training, reps, self._padding
            )
            return model.get_output()

    def _update_smoothed_mean(self, mean, values, indices=None, weight=0.005):
        if indices is None:
            return tf.compat.v1.assign(mean, (1.0 - weight) * mean + weight * values)
        current_values = tf.gather_nd(mean, indices)
        updates = weight * (values - current_values)
        return tf.compat.v1.scatter_nd_add(mean, indices, updates)
    ...
    def get_target_probs(self, sess, batch):
        ...
        return sess.run(
            [self.target_token_probs, self.target_sense_probs], feed_dict=feed_dict
        )

    def _add_get_mean_q(self):
        self._mean_q_tokens = tf.compat.v1.placeholder(tf.int32, [None])
    ...
    def _add_find_neighbours(self):
        self._interesting_ids = tf.compat.v1.placeholder(tf.int32, [None])
        self._n_neighbours = tf.compat.v1.placeholder(tf.int32, [])

        sense_indices = tf.nn.embedding_lookup(
            self._sense_indices, self._interesting_ids
        )
        interesting_embeddings = tf.nn.embedding_lookup(self._embeddings, sense_indices)
        interesting_embeddings = tf.reshape(
            interesting_embeddings, [-1, self._embedding_size]
        )
        interesting_norms = tf.norm(interesting_embeddings, axis=1)

        norms = tf.norm(self._embeddings, axis=1)

        # (n_interesting, vocab.size*n_senses)
        dot = tf.matmul(interesting_embeddings, self._embeddings, transpose_b=True)
        dot = dot / tf.expand_dims(interesting_norms, axis=1)
        dot = dot / tf.expand_dims(norms, axis=0)
        cosine_similarities = tf.reshape(
            dot, [-1, self._max_senses, self._total_senses]
        )
        mask = tf.concat(
            [tf.constant([2.0]), tf.zeros([self._total_senses - 1], dtype=tf.float32)],
            axis=0,
        )
        ...
        self._neighbour_tokens = tf.nn.embedding_lookup(self._sense_to_token, indices)[
            :, :, 1:
        ]
        self._neighbour_sense_nums = tf.nn.embedding_lookup(
            self._sense_to_sense_num, indices
        )[:, :, 1:]

    def get_neighbours(self, sess, tokens, n=10):
        feed_dict = {self._interesting_ids: tokens, self._n_neighbours: n}
...
class PolyLM(object):
        ...
        for i in range(self._n_towers):
            with tf.device("/gpu:%d" % i):
                with tf.compat.v1.variable_scope(
                    "polylm", reuse=tf.compat.v1.AUTO_REUSE
                ):
                    tower = PolyLMModel(
                        self._vocab, self._n_senses, self._options, training=training
                    )
    ...
    def attempt_restore(self, sess, model_dir, expect_exists):
        ...
        if path and tf.io.gfile.exists(path + ".index"):
            ...
            if expect_exists:
                ...
                raise Exception("There is no saved checkpoint at %s" % bad_path)
            else:
                logging.info(
                    "There is no saved checkpoint at %s. "
                    "Creating model with fresh parameters." % model_dir
                )
                sess.run(tf.compat.v1.global_variables_initializer())
        n_params = sum(
    ...
    def _train_on_batch(self, sess, batches, step_num):
        ...
        if step_num < self._options.lr_warmup_steps:
            ...
            lr_ratio = (self._options.n_batches - step_num) / self._options.n_batches
        else:
            lr_ratio = 1.0
    ...
    def train(self, corpus, sess, test_words=[]):
        ...
        # global_step = 1
        tokens_read = 0
        masking_policy = [float(x) for x in self._options.masking_policy.split()]
        logging.info(
            "Masking policy: %.2f [MASK], %.2f self, %.2f random"
            % (masking_policy[0], masking_policy[1], masking_policy[2])
        )
        ...
        while batch_num < self._options.n_batches:
            ...
            if batch_num % self._options.save_every == 0:
                ...
                self._saver.save(sess, checkpoint_path, global_step=self._global_step)

    def disambiguate(self, sess, batch, method="prediction"):
        return self._default_model.disambiguate(sess, batch, method=method)
    ...
    def get_top_k_substitutes(self, sess, reps, k):
        return self._default_model.get_top_k_substitutes(sess, reps, k)
    ...
    def display_words(self, sess, words):
        ...
        sense_stats = self._default_model._get_mean_sense_probs(sess, vocab_ids)
        ...
        for i, (vocab_id, word) in enumerate(zip(vocab_ids, words)):
            info = [
                "qd=%.4f, qp=%.4f" % (sense_stats["qd"][i, s], sense_stats["qp"][i, s])
